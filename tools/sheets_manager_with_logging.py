...
class GoogleSheetsManager:
    ...
    def load_tasks_from_sheet(self, sheet_name: str = "pm_tasks") -> List[Dict]:
        """タスクをシートから読み込み"""
        try:
    ...
    def update_task_status(self, task_id: int, status: str, sheet_name: str = "pm_tasks") -> bool:
        """タスクステータスを更新"""
        try:
            spreadsheet = self.gc.open_by_key(self.spreadsheet_id)
            worksheet = spreadsheet.worksheet(sheet_name)
            ...
            # タスクを検索
            tasks = self.load_tasks_from_sheet(sheet_name)
            target_row = None
    ...
    def log_task_execution(self, task_data: Dict) -> bool:
        ...
        try:
            ...
            log_row = [
                ...
                task_data.get('quality_evaluation', '')[:200]
            ]
            
            log_sheet.append_row(log_row)
            self.logger.info(f"✅ タスク {task_data.get('task_id')} の実行をログに記録 (品質: {task_data.get('quality_score', 'N/A')}/10)")
            return True
