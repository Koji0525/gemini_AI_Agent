...
class KnowledgeManager:
    ...
    def add_knowledge(
        self, content: str, source: str, metadata: Optional[Dict] = None, max_retries: int = 3
    ) -> str:
        """ナレッジを追加（完全スキーマ対応）"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                # スキーマに合わせて挿入
                # 必須カラムを特定
                insert_data = {"content": full_content}
                ...
                if "database is locked" in str(e) and attempt < max_retries - 1:
                    ...
                    print(
                        f"  ⏳ データベースロック - {wait_time}秒後にリトライ（{attempt + 1}/{max_retries}）"
                    )
