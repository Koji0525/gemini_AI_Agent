...
class OptimizedTaskExecutor:
    ...
    async def execute_all_pending(self, max_tasks: int = None) -> Dict:
        ...
        has_wp_task = any("wp" in t.get("required_role", "").lower() for t in pending)
        ...
        for i, task in enumerate(pending, 1):
            ...
            try:
                ...
                results["details"].append({"task_id": task_id, "role": role, "agent": handler_name, "success": success})
...
async def main():
    ...
    parser.add_argument(
        "--mode", choices=["test", "aggressive", "balanced", "safe"], default="balanced", help="レート制限モード"
    )
