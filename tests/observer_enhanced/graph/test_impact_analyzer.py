...
@pytest.mark.skip(reason="SystemGraphDB依存のため一時スキップ")
class TestImpactAnalyzer:
    ...
    def db_with_data(self):
        ...
        db.add_component(
            "TaskExecutor", {"type": "agent", "lines": 1200, "file": "agents/task_executor.py"}
        )
        db.add_component(
            "ReviewAgent", {"type": "agent", "lines": 650, "file": "agents/review_agent.py"}
        )
        db.add_component(
            "SheetsManager", {"type": "tool", "lines": 1150, "file": "tools/sheets_manager.py"}
        )
        db.add_component(
            "KnowledgeManager", {"type": "tool", "lines": 980, "file": "tools/knowledge_manager.py"}
        )
        db.add_component(
            "Dashboard",
            {"type": "service", "lines": 450, "file": "agents/observability/dashboard.py"},
        )
    ...
    def test_generate_test_recommendations(self, analyzer_with_data):
        ...
        result = analyzer_with_data.generate_test_recommendations(
            component_id="SheetsManager", change_lines=100
        )
    ...
    def test_recommendations_for_large_change(self, analyzer_with_data):
        """大規模変更時の推奨アクションテスト"""
        result = analyzer_with_data.generate_test_recommendations(
            component_id="SheetsManager", change_lines=250  # 大規模変更
        )
        ...
        if score["risk_level"] in ["high", "critical"]:
            ...
            assert (
                "���ールバック" in recommendations["rollback_plan"]
                or "revert" in recommendations["rollback_plan"].lower()
            )
    ...
    def test_performance_generate_recommendations(self, analyzer_with_data):
        ...
        analyzer_with_data.generate_test_recommendations("SheetsManager", 100)
