    ...
    async def execute_single_task(self, task: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.knowledge_manager:
                try:
                    knowledge = await self.knowledge_manager.search_relevant_knowledge(
                        task_type=task.get("task_type"), keywords=task.get("task_name")
                    )
                    if knowledge:
                        logger.info(f"📚 ナレッジ活用: {len(knowledge)}件")
