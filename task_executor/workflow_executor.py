...
class WorkflowExecutor:
    ...
    def __init__(self, task_executor: TaskExecutor, sheets_manager: GoogleSheetsManager, browser_controller=None):
        ...
        # ワークフロー統計
        self.workflow_stats = {"total_workflows": 0, "completed": 0, "failed": 0, "partial_success": 0}
    ...
    async def _execute_multilingual_workflow(self, task: Dict) -> Dict:
        ...
        try:
            ...
            if not base_result:
                return {"success": False, "error": f"{base_language} コンテンツ生成失敗"}
            ...
            # ========================================
            success_count = sum(
                1 for r in results.values() if r and (r is True or (hasattr(r, "get") and r.get("success")))
            )
        ...
        try:
            ...
            while iteration < max_iterations:
                ...
                if iteration == 1:
                    ...
                    content_task = {**task, "task_id": f"{task_id}_content_v{iteration}", "required_role": "content"}
                else:
            ...
            for i, step in enumerate(steps, 1):
                ...
                if step.get("use_previous_output") and accumulated_output:
                    ...
                    if "prompt" in step_task:
                        step_task["prompt"] = step_task["prompt"].replace("{previous_output}", str(prev_output))
    ...
    async def _execute_parallel_workflow(self, task: Dict) -> Dict:
        ...
        try:
            ...
            for i, parallel_task in enumerate(parallel_tasks, 1):
                parallel_task_config = {**task, **parallel_task, "task_id": f"{task_id}_parallel{i}"}
                coroutines.append(self.task_executor.execute_task(parallel_task_config))
