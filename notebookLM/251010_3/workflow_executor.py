...
class WorkflowExecutor:
    ...
    async def _execute_multilingual_workflow(self, task: Dict) -> Dict:
        ...
        try:
            ...
            # ステップ3: WordPress多言語投稿（オプション）
            if task.get('auto_publish', False):
                logger.info("--- ステップ3: WordPress多言語投稿 ---")
                ...
                for lang, content in contents.items():
                    ...
                    wp_task = {
                        ...
                        'description': f'{lang} コンテンツをWordPressに投稿',
            ...
            return {
                ...
                'results': results,
                'contents': contents,
                'summary': f'{success_count}/{total_count} 言語で成功',
