...
class GoogleSheetsManager:
    ...
    async def update_task_status(self, task_id: int, status: str, sheet_name: str = "pm_tasks") -> bool:
        ...
        try:
            ...
            if len(all_data) <= 1:
                logger.warning(f"📭 タスクシート '{sheet_name}' にデータがありません")
                return False
            ...
            if task_id_col is None:
                task_id_col = 0
                logger.info(f"🔍 タスクID列を自動検出: 列{task_id_col + 1}")
        
            if status_col is None:
                status_col = len(headers)
                task_sheet.update_cell(1, status_col + 1, 'status')
                logger.info(f"📝 status列を追加: 列{status_col + 1}")
            ...
            if not task_found:
                ...
                for row in all_data[1:]:
                    if len(row) > task_id_col and row[task_id_col]:
                        available_ids.append(str(row[task_id_col]).strip())
                
                logger.warning(f"📊 タスク検索失敗詳細:")
                logger.warning(f"   検索対象ID: '{task_id_str}'")
                logger.warning(f"   検索列: {task_id_col + 1}")
                logger.warning(f"   利用可能ID: {available_ids}")
                logger.warning(f"   総タスク数: {len(all_data) - 1}")
                return False
        
            # === パート4: ステータス更新 ===
            task_sheet.update_cell(row_index, status_col + 1, status)
            logger.info(f"✅ タスク {task_id} のステータスを '{status}' に更新（行 {row_index}）")
            return True
