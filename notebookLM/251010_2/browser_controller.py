...
"""ブラウザ制御クラス（Gemini拡張版：画像・テキスト両対応・DeepSeek対応）"""
import asyncio
import time
import json
import re
...
class BrowserController:
    ...
    async def _wait_for_generation_complete(self, max_wait: int = 120) -> bool:
        """
        後方互換性のためのラッパーメソッド
        TaskExecutor からの呼び出しに対応
        """
        logger.warning("⚠️ 非推奨メソッド _wait_for_generation_complete が呼び出されました")
        logger.info("💡 代わりに wait_for_text_generation メソッドを使用してください")
        return await self.wait_for_text_generation(max_wait)
    
    # === パート: send_prompt_with_wait メソッド追加（オプション）===
    ...
    async def extract_latest_text_response_gemini(self) -> str:
        """Geminiの最新のテキスト応答を抽出(コード完全性検証強化版)"""
        try:
            logger.info("=" * 60)
            ...
            for method in priority_order:
                if method in all_results and all_results[method] and len(all_results[method]) > 100:
                    ...
                    # === 🆕 新規追加: コードブロック完全性の検証 ===
                    code_block_validation = self._validate_code_block_completeness(text)
                    
                    if not code_block_validation['is_complete']:
                        logger.error(f"❌ {method}: コードブロックが不完全です")
                        logger.error(f"   理由: {code_block_validation['reason']}")
                        logger.error(f"   詳細: {code_block_validation['details']}")
                        
                        # 不完全なコードの証拠をログ出力
                        if code_block_validation.get('incomplete_blocks'):
                            for i, block in enumerate(code_block_validation['incomplete_blocks'], 1):
                                logger.error(f"   不完全ブロック{i}: {block['type']} - {block['snippet']}")
                        
                        continue  # このテキストは採用せず次の方法へ
                    
                    logger.info(f"\n🎯 採用: {method} ({len(text)}文字)")
                    logger.info(f"✅ コードブロック完全性: OK")
    ...
    def _validate_code_block_completeness(self, text: str) -> Dict:
        ...
        try:
            ...
            # === パート2: PHPコードの完全性チェック ===
            if '<?php' in text:
                php_open_count = text.count('<?php')
                php_close_count = text.count('?>')
            ...
            # === パート4: 文章の突然の終了チェック ===
            # 最後の文字が意味のある終了文字かチェック
            text_stripped = text.strip()
            if text_stripped:
                last_char = text_stripped[-1]
                
                # コードブロック以外で文章が途中で切れているパターン
                if last_char not in ['.', '。', '!', '！', '?', '？', '`', '}', ']', ')']:
                    # ただし、最後がコードブロックの閉じタグ ```の直後なら問題なし
