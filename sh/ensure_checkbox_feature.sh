...
echo ""
echo "📍 STEP 2: ファイル確認"
if [ -f "app/templates/knowledge_list.html" ]; then
    echo "  ✅ knowledge_list.html 存在"
    
    # チェックボックスコードが含まれているか確認
    if grep -q "entry-checkbox" app/templates/knowledge_list.html; then
        echo "  ✅ チェックボックスコード確認済み"
    else
        echo "  ⚠️  チェックボックスコードなし - 再作成します"
    fi
else
    echo "  ❌ knowledge_list.html が見つかりません"
fi

# knowledge_list.htmlを完全に再作成
echo ""
echo "📍 STEP 3: knowledge_list.html 完全再作成"
...
<div class="knowledge-list">
    ...
    <div class="info-bar">
        ...
        <div class="export-links">
            ...
            <a href="/manage" class="btn-small btn-danger">管理</a>
        </div>
    </div>
    
    <!-- 削除フォーム -->
    <form method="POST" action="/delete/single" id="delete-form">
        <div class="delete-bar">
            <button type="button" onclick="selectAll()" class="btn-small">全選択</button>
            <button type="button" onclick="deselectAll()" class="btn-small">全解除</button>
            <button type="submit" class="btn-small btn-danger" onclick="return confirmDelete()">選択を削除</button>
        ...
        <div class="table-container">
            <table class="knowledge-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <input type="checkbox" name="entry_ids" value="{{ entry['id'] }}" class="entry-checkbox" onchange="updateCount()">
                        </td>
                        {% for col in columns %}
                        <td>
                            {% if col == 'content' %}
...
function updateCount() {
    ...
    document.getElementById('selected-count').textContent = `${checkboxes.length}件選択`;
}
...
function confirmDelete() {
    ...
    return confirm(`選択した${count}件のエントリを削除します。よろしいですか？\n\n削除前に自動的にバックアップが作成されます。`);
}
</script>
...
echo "  ✅ knowledge_list.html 再作成完了"
...
# 行数確認
LINE_COUNT=$(wc -l < app/templates/knowledge_list.html)
echo "  📊 ファイル行数: ${LINE_COUNT}行"

if [ $LINE_COUNT -lt 100 ]; then
    echo "  ⚠️  ファイルが短すぎます - 確認してください"
fi
...
echo ""
echo "📖 確認項目:"
echo "   1. ブラウザで http://localhost:5000/knowledge を開く"
echo "   2. テーブルの左端にチェックボックスが表示される"
echo "   3. 「全選択」「全解除」「選択を削除」ボタンが表示される"
echo ""
