...
def change_settings():
    ...
    if choice == "1":
        new_content = new_content.replace("AUTO_SYNC_ENABLED = False", "AUTO_SYNC_ENABLED = True")
        print("✅ 自動同期を有効化しました")

    elif choice == "2":
        new_content = new_content.replace("AUTO_SYNC_ENABLED = True", "AUTO_SYNC_ENABLED = False")
        print("✅ 自動同期を無効化しました")
        ...
        try:
            ...
            new_content = new_content.replace(
                f"SYNC_INTERVAL_MINUTES = {interval}", f"SYNC_INTERVAL_MINUTES = {new_interval}"
            )
