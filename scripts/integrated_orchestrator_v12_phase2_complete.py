#!/usr/bin/env python3
"""
🎹 Integrated Orchestrator v12.0 (Phase 2 Day 1完成版)
Phase 2 Day 1: RetryManager + ErrorClassifier統合（完全修正版）
"""
import sys
...
class IntegratedOrchestrator:
    ...
    def __init__(self):
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        print("🚀 Integrated Orchestrator v12.0 初期化")
        print("   (Phase 2 Day 1: 自己修復機能統合完成版)")
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        ...
        # RetryConfig作成（正しい引数名で）
        self.retry_config = RetryConfig(
            max_attempts=3,  # max_retriesではなくmax_attempts
            base_delay=1.0,
            max_delay=60.0,
        ...
        # RetryManager初期化（sheets_managerを渡す）
        try:
            self.retry_manager = RetryManager(
                sheets_manager=self.sheets, config=self.retry_config  # 第一引数はsheets_manager  # 第二引数はconfig
            )
        ...
        try:
            ...
            if retry_result.success:
                ...
                if retry_result.total_attempts > 1:
                    print(f"  🔄 リトライ成功（試行回数: {retry_result.total_attempts}回）")
                    result["retried"] = True
                return result
            else:
                ...
                return {
                    "status": "error",
                    "error": str(retry_result.final_error),
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Integrated Orchestrator v12.0")
