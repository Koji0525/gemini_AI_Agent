    ...
    def sync_goals_to_dashboard(self):
        ...
        try:
            ...
            for row in goals_data[1:]:
                if (
                    len(row) > status_idx
                    and row[status_idx].lower() in ["active", "実行中", "in progress"]
                    and len(row) > description_idx
                ):
                    ...
                    active_goals.append(
                        {
                            ...
                            "name": (
                                row[description_idx][:100] + "..."
                                if len(row[description_idx]) > 100
                                else row[description_idx]
                            ),
