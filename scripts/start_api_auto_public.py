...
app = FastAPI(
    ...
    description="自動公開設定付きマルチエージェントシステムAPI",
    version="2.0.0",
)
...
def load_data():
    """データを読み込む"""
    global users_db, api_keys_db, tasks_db
    users_db = {}
    api_keys_db = {}
    tasks_db = {}
...
async def register_user(request: UserRegistration):
    try:
        ...
        return {
            ...
            "important": "このAPIキーをコピーして保存してください:",
            "your_api_key": api_key,
            "usage": "X-API-Keyヘッダーにこのキーを設定してAPIを利用",
        }
...
def set_port_public(port):
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"✅ ポート{port}を公開設定にしました")
        else:
            print(
                f"⚠️ 自動公開設定に失敗しました。手動でポート{port}を公開に設定してください"
            )
    except Exception:
        print(f"📝 ポート{port}を手動で公開に設定してください")
...
if __name__ == "__main__":
    ...
    print("🚀 自動公開設定付きマルチエージェントシステムAPI")
    print("=" * 60)
    print(f"📍 ポート: {port}")
    print("")

    # ポート公開設定を試行
    set_port_public(port)
    ...
    print(
        f"   🏠 APIルート: https://gory-cackle-69xw7jw55qxwc557w-{port}.app.github.dev/"
    )
    print("")
    print("💡 特徴:")
    print("   • データ永続化（再起動後も保持）")
    print("   • APIキー明確表示")
    print("   • 自動公開設定試行")
    print("")
    print("⏳ 起動中...")
