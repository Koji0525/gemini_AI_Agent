#!/usr/bin/env python3
"""
ナレッジベース用Google Sheetsスキーマ作成（v3 - 実際のAPI準拠）
"""
import sys
...
def get_or_create_sheet(client: gspread.Client, spreadsheet_id: str, sheet_name: str, headers: list):
    """シートを取得または作成（gspread直接使用）"""
    try:
        # スプレッドシートを開く
...
def create_knowledge_base_sheet(client, spreadsheet_id):
    ...
    sheet = get_or_create_sheet(client, spreadsheet_id, SHEET_NAME, HEADERS)
...
def create_learning_patterns_sheet(client, spreadsheet_id):
...
def create_success_recipes_sheet(client, spreadsheet_id):
...
def main():
    ...
    # GoogleSheetsManager初期化（gspreadクライアントを取得するため）
    ...
    # gspreadクライアントを取得
    # GoogleSheetsManagerが持っているclientを使用
    client = sheets_manager.client
    print(f"✅ gspreadクライアント取得: {type(client)}")
    print()
    ...
    results["knowledge_base"] = create_knowledge_base_sheet(client, spreadsheet_id)
    print()

    print("2️⃣  learning_patterns シート作成...")
    results["learning_patterns"] = create_learning_patterns_sheet(client, spreadsheet_id)
    print()

    print("3️⃣  success_recipes シート作成...")
    results["success_recipes"] = create_success_recipes_sheet(client, spreadsheet_id)
    print()
    ...
    if all_success:
        ...
        print()
        print("次のステップ:")
        print("STEP 8.2: KnowledgeBaseManager実装（所要時間: 4時間）")
    else:
        print()
