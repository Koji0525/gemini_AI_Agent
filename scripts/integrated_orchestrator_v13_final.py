#!/usr/bin/env python3
"""
🎹 Integrated Orchestrator v13.0 (完全修正版)
Phase 2 Day 1: 互換性問題を完全解決
"""
import sys
...
# 互換性アダプター
from agents.self_healing.adapters.sheets_adapter import SheetsManagerAdapter
...
class IntegratedOrchestrator:
    """24時間自律開発の統合制御ハブ（完全互換性対応）"""

    def __init__(self):
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        print("🚀 Integrated Orchestrator v13.0 初期化")
        print("   (Phase 2: 互換性問題完全解決版)")
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        ...
        # 🆕 gc属性が存在しない場合は追加
        if not hasattr(self.sheets, "gc"):
            if hasattr(self.sheets, "service"):
                self.sheets.gc = self.sheets.service
                print("  🔧 gc属性を追加（互換性対応）")
        ...
        # RetryManager初期化（アダプター使用）
        try:
            # アダプターでラップ
            sheets_adapter = SheetsManagerAdapter(self.sheets)

            self.retry_manager = RetryManager(sheets_manager=sheets_adapter, config=self.retry_config)
            print("  ✅ RetryManager 初期化完了（アダプター使用）")
        except Exception as e:
            print(f"  ⚠️ RetryManager初期化失敗: {e}")
            # gc属性を直接追加して再試行
            try:
                if hasattr(self.sheets, "service") and not hasattr(self.sheets, "gc"):
                    self.sheets.gc = self.sheets.service

                self.retry_manager = RetryManager(sheets_manager=self.sheets, config=self.retry_config)
                print("  ✅ RetryManager 初期化完了（gc属性追加）")
            except Exception as e2:
                print(f"  ⚠️ RetryManager初期化再試行も失敗: {e2}")
                print("     自己修復機能なしで続行します")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Integrated Orchestrator v13.0")
