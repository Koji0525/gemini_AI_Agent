#!/usr/bin/env python3
"""
ナレッジベース用Google Sheetsスキーマ作成（最終版）
"""
import sys
...
def get_or_create_sheet(gc: gspread.Client, spreadsheet_id: str, sheet_name: str, headers: list):
    """シートを取得または作成"""
    try:
        # スプレッドシートを開く
...
def create_knowledge_base_sheet(gc, spreadsheet_id):
    ...
    sheet = get_or_create_sheet(gc, spreadsheet_id, SHEET_NAME, HEADERS)

    if sheet and not sheet.row_values(2):
        sample_data = [
            [
                "KB_SAMPLE_001",
                "2025-10-29 11:30:00",
...
def create_learning_patterns_sheet(gc, spreadsheet_id):
...
def create_success_recipes_sheet(gc, spreadsheet_id):
...
def main():
    ...
    print(f"⚙️  SPREADSHEET_ID: {spreadsheet_id[:20]}...")
    print(f"⚙️  SERVICE_ACCOUNT: {service_account_file}")
    print()
    ...
    # gspreadクライアントを取得（正しい属性名: gc）
    gc = sheets_manager.gc
    print(f"✅ gspreadクライアント取得: {type(gc).__name__}")
    print()
    ...
    results["knowledge_base"] = create_knowledge_base_sheet(gc, spreadsheet_id)
    print()

    print("2️⃣  learning_patterns シート作成...")
    results["learning_patterns"] = create_learning_patterns_sheet(gc, spreadsheet_id)
    print()

    print("3️⃣  success_recipes シート作成...")
    results["success_recipes"] = create_success_recipes_sheet(gc, spreadsheet_id)
    print()
    ...
    if all_success:
        ...
        print("📊 作成されたシート")
        print("=" * 60)
        print("✅ knowledge_base (13列) - 統合ナレッジベース")
        print("✅ learning_patterns (8列) - 学習パターン")
        print("✅ success_recipes (10列) - 成功レシピ")
        print()
        print("🔗 Google Sheetsで確認:")
        print(f"   https://docs.google.com/spreadsheets/d/{spreadsheet_id}")
        print()
        print("=" * 60)
        print("📋 次のステップ: STEP 8.2")
        print("=" * 60)
        print("タスク: KnowledgeBaseManager実装")
        print("所要時間: 4時間")
        print("優先度: 🔴 最高")
        print()
    else:
