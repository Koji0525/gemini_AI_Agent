...
class HumanInteractionAgent:
    ...
    def __init__(self, repo_name: str = None):
        """
        Args:
            repo_name: リポジトリ名（オプション、未指定時は環境変数GITHUB_REPOから取得）（例: "Koji0525/gemini_AI_Agent"）
        """
        # 環境変数から取得（フォールバック）
        ...
        if not repo_name:
            raise ValueError(
                "リポジトリ名が指定されていません\n"
                "1. HumanInteractionAgent(repo_name='owner/repo') または\n"
                "2. 環境変数 GITHUB_REPO='owner/repo' を設定してください"
            )
    ...
    async def monitor_issues(self, check_interval: int = 60):
        ...
        while self.running:
            ...
            try:
                ...
                issues = self.repo.get_issues(state="open", labels=["bot-control"])
...
def main():
    ...
    parser.add_argument(
        "--test-duration", type=int, default=0, help="テスト実行時間（分）。0=無制限"
    )
