...
class WordPressPostCreator:
    ...
    async def create_post(self, page: Page, task: Dict) -> Dict:
        ...
        try:
            ...
            if target_task_id:
                ...
                post_content = await TaskContentFetcher.get_task_content(self.sheets_manager, target_task_id)
    ...
    async def _save_draft(self, page: Page) -> bool:
        """下書き保存"""
        save_selectors = ['button[aria-label="下書き保存"]', 'button:has-text("下書き保存")', ".editor-post-save-draft"]
