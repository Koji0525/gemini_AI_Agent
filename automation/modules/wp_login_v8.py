"""
WordPress自動ログイン - setup_browser引数対応版
...
class WPAutoLogin:
    ...
    async def setup(self, headless=True):
        ...
        try:
            ...
            await self.browser.setup_browser(headless=headless)
            print(f"✅ ブラウザセットアップ完了 (headless={headless})")
            return True
    ...
    async def run_full_test(self, headless=True):
        """完全テスト実行"""
        print("=" * 60)
        ...
        # セットアップ
        print(f"\n🔧 ブラウザセットアップ中 (headless={headless})...")
        if not await self.setup(headless=headless):
            return False
...
async def main():
    ...
    # まずheadless=Trueで試す（ヘッドレスモード）
    print("🎯 第1試行: headless=True (ヘッドレスモード)")
    success = await login_system.run_full_test(headless=True)

    if not success:
        print("\n🔄 第2試行: headless=False (GUIモード)")
        # ブラウザをクリーンアップして再試行
        if login_system.browser:
            await login_system.browser.cleanup()
            login_system.browser = None
        success = await login_system.run_full_test(headless=False)
    ...
    if success:
        ...
        print("  ✅ BrowserController.setup_browser()にheadless引数追加")
