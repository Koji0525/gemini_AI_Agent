...
class WPAutoLogin:
    ...
    async def run_full_test(self, headless=False):
        """完全テスト実行"""
        print("=" * 60)
...
async def main():
    ...
    # まずheadless=Falseで試す（GUIモード）
    print("🎯 第1試行: headless=False (GUIモード)")
    success = await login_system.run_full_test(headless=False)

    if not success:
        print("\n🔄 第2試行: headless=True (ヘッドレスモード)")
