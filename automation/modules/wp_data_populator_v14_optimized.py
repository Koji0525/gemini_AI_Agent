"""
WordPress企業データ登録モジュール V14 - 最適化版
Day 3: タイムアウト問題とネットワーク最適化
"""
...
class WPDataPopulatorV14:
    """WordPress企業データ登録 V14 - 最適化版"""
    ...
    async def debug_page_state(self, step: str):
        ...
        try:
            ...
            # ページの読み込み状態
            try:
                load_state = await self.browser.page.evaluate(
                    """
                    () => {
                        return {
                ...
                print(f"   📄 ページ状態: {load_state}")
            except Exception as e:
                print(f"   ⚠️ ページ状態取得エラー: {e}")
    ...
    async def setup_robust(self) -> bool:
        """ブラウザセットアップ - ロバスト版"""
        max_retries = 3
        for attempt in range(max_retries):
            try:
                print(f"🔄 ブラウザセットアップ試行 {attempt + 1}/{max_retries}")

                self.browser = BrowserController()
                print("✅ BrowserControllerインスタンス作成完了")

                await self.browser.setup_browser(headless=True)
                print("✅ setup_browser() 実行完了")

                # ページオブジェクトの確認
                if hasattr(self.browser, "page") and self.browser.page:
                    print("✅ ページオブジェクトを確認")

                    # ブラウザ設定の適用（エラーを無視して続行）
                    try:
                        await self.browser.page.set_viewport_size({"width": 1920, "height": 1080})
                        print("✅ ビューポート設定完了")
                    except Exception as e:
                        print(f"⚠️ ビューポート設定スキップ: {e}")
                    ...
                    try:
                        ...
                        print(f"⚠️ タイムアウト設定スキップ: {e}")

                print("✅ ブラウザセットアップ完了")
                return True

            except Exception as e:
                print(f"❌ ブラウザセットアップ試行 {attempt + 1} 失敗: {e}")
    ...
    async def login_to_wordpress_optimized(self) -> bool:
        """WordPressにログイン - 最適化版"""
        max_retries = 3
        for attempt in range(max_retries):
            try:
                print(f"🔐 WordPressログイン試行 {attempt + 1}/{max_retries}: {self.wp_url}")

                # ログインページに移動（より寛容な設定）
                await self.browser.page.goto(
                    f"{self.wp_url}/wp-login.php",
                    wait_until="domcontentloaded",  # networkidle より軽量
                ...
                # ログイン成功確認（複数方法、段階的）
                success_indicators = [
                    ("#wpadminbar", 10000, "wpadminbar"),
                    (".wp-admin", 8000, "admin要素"),
                    ("#adminmenu", 8000, "adminメニュー"),
                    ("#dashboard-widgets", 5000, "ダッシュボードウィジェット"),
                ]

                for selector, timeout, description in success_indicators:
                    try:
                        await self.browser.page.wait_for_selector(selector, timeout=timeout)
                        print(f"✅ WordPressログイン成功 ({description})")
                        return True
                    except:
                        continue
                ...
                if "ダッシュボード" in page_title or "Dashboard" in page_title:
                    print("✅ WordPressログイン成功 (タイトル確認)")
                    return True

                print(f"❌ ログイン成功確認できません (試行 {attempt + 1})")

            except Exception as e:
                print(f"❌ WordPressログイン試行 {attempt + 1} 失敗: {e}")

            # リトライ前の待機
            if attempt < max_retries - 1:
                print(f"⏳ ログインリトライまで5秒待機...")
    ...
    async def navigate_to_new_post_optimized(self) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                print(f"📝 新規投稿ページ移動試行 {attempt + 1}/{max_retries}")
                ...
                # 投稿作成ページへ移動（最適化された設定）
                navigation_options = {
                    "wait_until": "domcontentloaded",  # networkidle から変更
                    "timeout": 45000,  # 45秒に短縮
                }

                target_url = f"{self.wp_url}/wp-admin/post-new.php"
                print(f"   🎯 移動先: {target_url}")

                await self.browser.page.goto(target_url, **navigation_options)
                print("   ✅ ページ移動完了")

                # ページの基本読み込みを待機
                await self.browser.page.wait_for_timeout(3000)

                # エディタの読み込み状態を確認（段階的アプローチ）
                editor_loaded = await self.wait_for_editor_loading()
                if not editor_loaded:
                    print("   ⚠️ エディタの完全な読み込みを確認できませんでしたが、続行します")
                ...
                print(f"❌ 新規投稿ページ移動試行 {attempt + 1} 失敗: {e}")

                # エラーの詳細情報を取得
                await self.debug_page_state("移動失敗時")
                ...
                if attempt < max_retries - 1:
                    ...
                    try:
                        await self.browser.page.goto(
                            f"{self.wp_url}/wp-admin/", wait_until="domcontentloaded", timeout=30000
                        )
                        print("   �� ダッシュボードに戻りました")
                    except:
                        print("   ⚠️ ダッシュボードへの戻りに失敗しました")
                else:
                    print("❌ すべての移動試行が失敗しました")
                    return False

        return False

    async def wait_for_editor_loading(self) -> bool:
        """エディタの読み込みを待機"""
        try:
            print("   ⏳ エディタの読み込みを待機中...")
            ...
            editor_selectors = [
                ...
                "#title",  # クラシックエディタタイトル
                ".block-editor-page",  # ブロックエディタコンテナ
                ".wp-editor-wrap",  # クラシックエディタコンテナ
                "#content",  # コンテンツエリア
                ".edit-post-layout",  # 編集レイアウト
            ]
            ...
            for selector in editor_selectors:
                try:
                    element = await self.browser.page.wait_for_selector(
                        selector, timeout=10000, state="attached"  # DOMに接続されていることのみ確認
                    )
                    if element:
            ...
            if loaded_selectors:
                print(f"   ✅ エディタ読み込み完了: {len(loaded_selectors)}/{len(editor_selectors)} 要素を確認")
                return True
            else:
                print("   ⚠️ 主要なエディタ要素を確認できませんでした")
                return False

        except Exception as e:
            print(f"   ⚠️ エディタ待機中にエラー: {e}")
            return False

    # 以下のメソッドはV13と同じ（企業データ登録、コンテンツ入力など）
...
"""
    ...
    async def run(self) -> Dict:
        ...
        try:
            print("=" * 60)
            print("🏢 WordPress企業データ登録 - Day 3 (最適化版 V14)")
            print("=" * 60)
            ...
            if not await self.setup_robust():
                self.results["error"] = "ブラウザセットアップ失敗"
                return self.results

            if not await self.login_to_wordpress_optimized():
                self.results["error"] = "WordPressログイン失敗"
                return self.results

            results = await self.register_all_companies()

            result_file = f"{self.log_dir}/registration_results_v14.json"
...
async def main():
    """メイン実行関数"""
    populator = WPDataPopulatorV14()
    results = await populator.run()

    print("\n" + "=" * 60)
    print("📊 Day 3 実行結果サマリー (最適化版 V14)")
    print("=" * 60)
