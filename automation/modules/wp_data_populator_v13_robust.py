"""
WordPress企業データ登録モジュール V13 - ロバスト版
Day 3: ページ初期化問題を根本解決
"""
...
class WPDataPopulatorV13:
    """WordPress企業データ登録 V13 - ロバスト版"""
    ...
    async def debug_browser_state(self):
        """ブラウザ状態をデバッグ"""
        try:
            print("🔍 ブラウザ状態デバッグ:")

            if not self.browser:
                print("   ❌ BrowserControllerインスタンスがありません")
                return

            # BrowserControllerの属性を確認
            attrs = ["page", "browser", "context", "cookies"]
            for attr in attrs:
                if hasattr(self.browser, attr):
                    value = getattr(self.browser, attr)
            ...
            if hasattr(self.browser, "page") and self.browser.page:
                ...
                print("   ❌ ページオブジェクトが利用できません")

        except Exception as e:
            print(f"   ❌ ブラウザ状態デバッグエラー: {e}")
    ...
    async def setup_robust(self) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # ページオブジェクトの確認（段階的アプローチ）
                print("🔍 ページオブジェクトの状態確認...")

                # 方法1: 直接アクセス
                if hasattr(self.browser, "page") and self.browser.page:
                    print("✅ ページオブジェクトを直接確認")
                    page = self.browser.page
                else:
                    # 方法2: 代替属性を確認
                    print("⚠️ 直接ページオブジェクトが見つかりません。代替方法を試行...")
                    await self.debug_browser_state()

                    # ブラウザ状態を確認
                    browser_attrs = [attr for attr in dir(self.browser) if not attr.startswith("_")]
                    print(f"🔍 BrowserControllerの利用可能な属性: {browser_attrs}")

                    # よくある属性名を試す
                    possible_page_attrs = ["page", "_page", "browser_page", "current_page"]
                    for attr in possible_page_attrs:
                        if hasattr(self.browser, attr):
                            page = getattr(self.browser, attr)
                            if page is not None:
                                print(f"✅ 代替属性 '{attr}' からページオブジェクトを取得")
                                self.browser.page = page  # 標準属性に設定
                                break
                    else:
                        print("❌ どの属性からもページオブジェクトを取得できません")
                        raise Exception("ページオブジェクトが見つかりません")

                # ページが利用可能か確認
                if not self.browser.page:
                    print("❌ ページオブジェクトがNoneです")
                    raise Exception("ページオブジェクトがNone")
                ...
                try:
                    ...
                    print(f"✅ ページ動作確認成功: {current_url}")
                except Exception as nav_error:
                    print(f"⚠️ ページナビゲーション確認で警告: {nav_error}")
                    # ナビゲーション失敗しても続行可能な場合がある
                ...
                try:
                    ...
                    print(f"⚠️ ブラウザ設定適用で警告: {config_error}")
                    # 設定失敗しても続行可能
    ...
    async def login_to_wordpress(self) -> bool:
        ...
        try:
            ...
            # ログイン成功確認（複数方法）
            success_indicators = [
                ("#wpadminbar", 15000, "wpadminbar"),
                (".wp-admin", 10000, "admin要素"),
                ("#adminmenu", 10000, "adminメニュー"),
            ]
    ...
    # 以下のメソッドはV12と同じ（デバッグ、ナビゲーション、コンテンツ入力など）
...
"""
    ...
    async def run(self) -> Dict:
        ...
        try:
            print("=" * 60)
            print("🏢 WordPress企業データ登録 - Day 3 (ロバスト版 V13)")
            print("=" * 60)
            ...
            result_file = f"{self.log_dir}/registration_results_v13.json"
...
async def main():
    """メイン実行関数"""
    populator = WPDataPopulatorV13()
    results = await populator.run()

    print("\n" + "=" * 60)
    print("📊 Day 3 実行結果サマリー (ロバスト版 V13)")
    print("=" * 60)
