"""
WordPress企業データ登録モジュール V4 - 安定版
Day 3: 新規投稿ページ移動のタイムアウト問題を修正
"""
...
class WPDataPopulatorV4:
    """WordPress企業データ登録 V4 - 安定版"""
    ...
    async def navigate_to_new_post_with_retry(self, max_retries: int = 3) -> bool:
        """新規投稿ページに移動 - リトライ機能付き"""
        for attempt in range(max_retries):
            try:
                print(f"📝 新規投稿ページへ移動 (試行 {attempt + 1}/{max_retries})")
                ...
                try:
                    ...
                    print("✅ 直接アクセス成功")
                    return await self.verify_editor_loaded()
                except Exception as direct_error:
                    print(f"⚠️ 直接アクセス失敗: {direct_error}")

                # 方法2: ダッシュボード経由
                try:
                    print("🔄 ダッシュボード経由でアクセスを試みます")
                    await self.browser.page.goto(
                        f"{self.wp_url}/wp-admin/", wait_until="domcontentloaded", timeout=30000
                    )

                    # 投稿メニューをクリック
                    posts_menu = await self.browser.page.query_selector("#menu-posts")
                    if posts_menu:
                        await posts_menu.click()
                        await self.browser.page.wait_for_timeout(2000)

                    # 新規追加ボタンをクリック
                    add_new_btn = await self.browser.page.query_selector('a[href*="post-new.php"]')
                    if add_new_btn:
                        await add_new_btn.click()
                        await self.browser.page.wait_for_selector("#title", timeout=30000)
                        print("✅ ダッシュボード経由アクセス成功")
                        return await self.verify_editor_loaded()

                except Exception as dashboard_error:
                    print(f"⚠️ ダッシュボード経由アクセス失敗: {dashboard_error}")

                # 方法3: JavaScriptナビゲーション
                try:
                    print("🔄 JavaScriptナビゲーションを試みます")
                    js_success = await self.browser.page.evaluate(
                        f"""() => {{
                        window.location.href = '{self.wp_url}/wp-admin/post-new.php';
                        return true;
                    }}"""
                    )

                    if js_success:
                        await self.browser.page.wait_for_selector("body", timeout=30000)
                        print("✅ JavaScriptナビゲーション成功")
                        return await self.verify_editor_loaded()

                except Exception as js_error:
                    print(f"⚠️ JavaScriptナビゲーション失敗: {js_error}")

                # リトライ前の待機
                if attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 5
                    print(f"⏳ {wait_time}秒待機して再試行...")
                    await self.browser.page.wait_for_timeout(wait_time * 1000)

            except Exception as e:
                print(f"❌ 新規投稿ページ移動失敗 (試行 {attempt + 1}): {e}")
                if attempt < max_retries - 1:
                    await self.browser.page.wait_for_timeout(5000)

        print("❌ 全ての新規投稿ページ移動方法が失敗しました")
        return False

    async def verify_editor_loaded(self) -> bool:
        """エディタが正常に読み込まれたか確認"""
        try:
            # 複数のエディタ要素を確認
            editor_selectors = [
                ...
                "#content",  # クラシックエディタ本文
                ".editor-post-text-editor",  # ブロックエディタ本文
                "body.wp-admin",  # 管理画面ボディ
            ]

            for selector in editor_selectors:
                try:
                    await self.browser.page.wait_for_selector(selector, timeout=10000)
    ...
    async def create_company_post(self, company: Dict) -> bool:
        ...
        try:
            ...
            for selector in title_selectors:
                ...
                if title_element:
                    # 既存の内容をクリアしてから入力
                    await title_element.click(click_count=3)  # 全選択
                    await title_element.press("Backspace")
    ...
    async def add_content_to_editor(self, content: str) -> bool:
        ...
        try:
            ...
            if classic_editor:
                await classic_editor.click(click_count=3)  # 全選択
                await classic_editor.press("Backspace")
            ...
            if block_editor:
                ...
                await block_editor.click(click_count=3)  # 全選択
                await block_editor.press("Backspace")
                await block_editor.fill(content)
                print("✅ ブロックエディタにコンテンツ入力")
                return True

            # ビジュアルエディタのiframe
            iframe = await self.browser.page.query_selector("#content_ifr")
            if iframe:
                frame = await iframe.content_frame()
                if frame:
                    body = await frame.query_selector("body")
                    if body:
                        await body.click(click_count=3)
                        await body.press("Backspace")
                        await body.type(content)
                        print("✅ ビジュアルエディタにコンテンツ入力")
                        return True
        ...
        try:
            ...
            if publish_btn:
                ...
                if not is_disabled:
                    ...
                    # 成功メッセージを確認
                    success_selectors = [".updated", ".notice-success", ".components-snackbar"]
                    for selector in success_selectors:
                        success_msg = await self.browser.page.query_selector(selector)
                        if success_msg:
                            msg_text = await success_msg.text_content()
                            print(f"✅ 保存成功: {msg_text}")
                            return True

                    # メッセージがなくてもURLが変わっていれば成功とみなす
                    if self.browser.page.url != current_url:
                        print("✅ URLが変更されたため保存成功と判断")
                        return True
...
"""
    ...
    async def register_all_companies(self) -> Dict:
        ...
        for i, company in enumerate(companies, 1):
            ...
            # 新規投稿ページに移動（リトライ機能付き）
            if not await self.navigate_to_new_post_with_retry():
    ...
    async def run(self) -> Dict:
        ...
        try:
            print("=" * 60)
            print("🏢 WordPress企業データ登録 - Day 3 (安定版 V4)")
            print("=" * 60)
            ...
            # 結果保存
            result_file = f"{self.log_dir}/registration_results_v4.json"
...
async def main():
    """メイン実行関数"""
    populator = WPDataPopulatorV4()
    results = await populator.run()

    print("\n" + "=" * 60)
    print("📊 Day 3 実行結果サマリー (安定版 V4)")
    print("=" * 60)
    ...
    if results["successful_registrations"] >= 3:  # 60%以上成功でOK
        ...
        print("❌ Day 3 要改善")
        print("📄 詳細ログ: automation/logs/day3/registration_results_v4.json")
        return 1
