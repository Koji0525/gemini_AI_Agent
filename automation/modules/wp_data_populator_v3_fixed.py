...
class WPDataPopulatorV3:
    ...
    async def setup(self) -> bool:
        ...
        try:
            ...
            await self.browser.setup_browser(headless=False)  # デバッグ用にヘッドレスをオフ
            print("✅ ブラウザセットアップ完了")
            return True
        except Exception as e:
    ...
    async def create_company_post(self, company: Dict) -> bool:
        ...
        try:
            ...
            # 成功確認
            success_msg = await self.browser.page.query_selector(".updated, .notice-success, .components-snackbar")
            if success_msg:
                msg_text = await success_msg.text_content()
                print(f"✅ 企業データ登録成功: {company['name']} - {msg_text}")
                return True
            else:
                # エラーメッセージ確認
                ...
                if error_msg:
                    ...
                    print(f"❌ 保存エラー: {err_text}")
                else:
                    print("⚠️ 保存結果が不明ですが、続行します")
