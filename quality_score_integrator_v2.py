...
class QualityScoreIntegratorV2:
    ...
    def get_actual_quality_scores(self, force_refresh: bool = False) -> Dict[str, float]:
        ...
        if (
            not force_refresh
            and self._quality_cache is not None
            ...
            for log in logs:
                ...
                if quality_score > 0:
                    ...
                    if str(task_id) not in actual_scores:
                        ...
                        actual_scores[str(task_id)] = max(actual_scores[str(task_id)], quality_score)
    ...
    def calculate_goal_quality(self, goal_id: str, tasks: List[Dict], use_cache: bool = True) -> float:
        """
        親目標の平均品質スコアを計算
