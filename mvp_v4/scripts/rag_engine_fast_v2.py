...
class FastRAGEngine:
    ...
    def __init__(self, persist_dir="mvp_v4/chroma_persist"):
        ...
        # コレクション取得または作成
        try:
            self.collection = self.client.get_collection("knowledge_base_v2")
            print("✅ 既存データベースを読み込みました（高速起動）")
        except:
            self.collection = self.client.create_collection(
                name="knowledge_base_v2", metadata={"hnsw:space": "cosine"}
            )
            print("📊 新規データベース作成")
    ...
    def _sanitize_metadata(self, metadata):
        """
        メタデータをChromaDB対応型に変換

        ChromaDB制約:
        - 値は str, int, float, bool, None のみ
        - list, dict は不可
        """
        sanitized = {}
        ...
        if unique_kb:
            ...
            for i in range(0, len(unique_kb), batch_size):
                ...
                for kb in batch:
                    ...
                    # メタデータを型変換
                    sanitized_metadata = self._sanitize_metadata(kb)
