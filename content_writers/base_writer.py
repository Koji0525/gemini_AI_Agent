...
class BaseContentWriter(ABC):
    ...
    async def process_task(self, task: Dict) -> Dict:
        ...
        try:
            ...
            if not success:
                return {"success": False, "error": f"{lang_name}ライターAI: タイムアウト"}

            # 応答を取得
            article_html = await self.browser.extract_latest_text_response()

            if not article_html:
                return {"success": False, "error": f"{lang_name}ライターAI: 記事取得失敗"}
...
**{self.get_language_name()}で、HTMLタグを使用した完全な記事を最後まで書き切ってください。**"""
    ...
    def _parse_task_requirements(self, description: str) -> Dict:
        ...
        return {"url": url, "target_audience": "ウズベキスタンのM&Aに興味があるビジネスオーナー"}
    ...
    async def _save_article(self, task: Dict, html_content: str, title: str) -> Dict:
        ...
        # JSON保存
        json_filename = (
            f"article_{self.get_language_code()}_{task['task_id']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        json_output_path = self.output_folder / json_filename

        # HTML保存
        html_filename = (
            f"article_{self.get_language_code()}_{task['task_id']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        )
