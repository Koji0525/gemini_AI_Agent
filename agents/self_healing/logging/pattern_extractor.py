...
class PatternExtractor:
    ...
    def _extract_success_patterns(self, integrated_logs: Dict[str, IntegratedLog]) -> List[KnowledgePattern]:
        """成功パターンを抽出"""
        patterns = []
            ...
            for exec_log in log.execution_logs:
                if exec_log.get("status") == "completed":
                    ...
                    if quality >= self.SUCCESS_MIN_QUALITY:
                        ...
                        type_groups[task_type].append({"task_id": task_id, "quality": quality, "log": exec_log})
    ...
    def _extract_failure_patterns(self, integrated_logs: Dict[str, IntegratedLog]) -> List[KnowledgePattern]:
        """失敗パターンを抽出"""
        patterns = []
        ...
        for task_id, log in integrated_logs.items():
            ...
            for exec_log in log.execution_logs:
                if exec_log.get("status") in ["failed", "error"]:
                    ...
                    error_groups[error_type].append({"task_id": task_id, "error_msg": error_msg, "log": exec_log})
            ...
            for retry_log in log.retry_logs:
                if not retry_log.get("success"):
                    ...
                    error_groups[error_type].append(
                        {"task_id": task_id, "error_msg": retry_log.get("error_message", ""), "log": retry_log}
                    )
    ...
    def _extract_fix_recipes(self, integrated_logs: Dict[str, IntegratedLog]) -> List[KnowledgePattern]:
        """修正レシピを抽出（コンテキストログから）"""
        patterns = []
        ...
        for task_id, log in integrated_logs.items():
            for context_log in log.context_logs:
                ...
                if pattern_name:
                    recipe_groups[pattern_name].append({"task_id": task_id, "context": context_log})
        ...
        for pattern_name, recipes in recipe_groups.items():
            ...
            # システム状態を適用条件として設定
            try:
                pattern.applicable_conditions = json.loads(context.get("system_state", "{}"))
            except:
    ...
    def _extract_high_quality_patterns(self, integrated_logs: Dict[str, IntegratedLog]) -> List[KnowledgePattern]:
        """高品質タスクの特徴を抽出"""
        patterns = []
