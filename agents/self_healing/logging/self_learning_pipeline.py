...
class SelfLearningPipeline:
    ...
    async def run_learning_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not patterns:
                ...
                return {"success": True, "patterns_found": 0, "patterns_saved": 0, "message": "No new patterns found"}
    ...
    def get_learning_recommendations(self) -> List[str]:
        ...
        if total < 10:
            recommendations.append("ナレッジが少なすぎます。より多くのタスクを実行してデータを蓄積してください。")

        if stats.get("success_patterns", 0) == 0:
            recommendations.append("成功パターンがありません。品質スコア8以上のタスクを3件以上実行してください。")

        if stats.get("failure_patterns", 0) > stats.get("fix_recipes", 0):
            recommendations.append(
                "失敗パターンに対する修正レシピが不足しています。" "エラー発生時に判断プロセスを記録してください。"
            )

        if not recommendations:
            recommendations.append("ナレッジベースは健全です。定期的な学習サイクルを継続してください。")
