...
class IntelligentFeedbackGenerator:
    ...
    def __init__(self, kb_manager=None, decision_system=None, use_gemini: bool = True):
        """
        初期化
    ...
    def generate_feedback_from_knowledge(self, limit: int = 5) -> List[FeedbackProposal]:
        """
        ナレッジベースから改善提案を生成
    ...
    def _generate_health_based_feedback(self, health: Dict[str, Any]) -> List[FeedbackProposal]:
        """健全性分析から提案生成"""
        proposals = []
            ...
            try:
                ...
                if success_count >= 5:  # 5回以上成功
                    proposal = FeedbackProposal(
                        ...
                        confidence=0.7,
                        actionable_steps=["類似タスクを特定", "成功パターンを適用", "効果を測定"],
    ...
    async def generate_gemini_feedback(self, context: str, knowledge_summary: str) -> Optional[str]:
        """
        Gemini APIで改善提案を生成
...
"""
    ...
    def format_feedback_report(self, proposals: List[FeedbackProposal]) -> str:
        """
        フィードバックレポートをフォーマット
