...
class SheetsAdapter:
    ...
    def get_recent_retries(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 辞書形式に変換
            headers = [
                'retry_id', 'timestamp', 'task_name', 'attempt_number',
                'error_type', 'error_message', 'strategy_used',
                'wait_time_sec', 'status', 'duration_sec'
            ]
        ...
        try:
            ...
            for row in all_values:
                ...
                stats['by_strategy'][strategy] = \
                    stats['by_strategy'].get(strategy, 0) + 1
...
class RetryHistoryManager:
    ...
    def show_recent_retries(self, limit: int = 5):
        ...
        for retry in retries:
            ...
            print(f"  試行: {retry['attempt_number']}回目 | "
                  f"エラー: {retry['error_type']} | "
                  f"戦略: {retry['strategy_used']}")
            print(f"  結果: {retry['status']} | "
    ...
    def show_stats(self, hours: int = 24):
        ...
        print(f"成功: {stats['success_count']}回 / "
              f"失敗: {stats['failure_count']}回")
