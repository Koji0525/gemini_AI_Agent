...
class AgentTemplate(ABC):
    ...
    async def run(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "result": result, "agent": self.metadata.name, "version": self.metadata.version}

        except Exception as e:
            # 失敗カウント
            self.failure_count += 1

            return {"success": False, "error": str(e), "agent": self.metadata.name, "version": self.metadata.version}
