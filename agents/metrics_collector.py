...
class MetricsCollectorAgent:
    ...
    def __init__(self, storage_dir: str = ".metrics"):
        ...
        # 修正タイプ別統計
        self.fix_stats: Dict[str, Dict[str, int]] = defaultdict(lambda: {"attempted": 0, "succeeded": 0, "failed": 0})
    ...
    def record_metric(
        self, name: str, value: float, metric_type: MetricType = MetricType.GAUGE, tags: Dict[str, str] = None
    ):
        """
        メトリクスを記録
        ...
        Args:
            ...
            tags: タグ
        """
        entry = MetricEntry(name=name, value=value, type=metric_type, timestamp=datetime.now(), tags=tags or {})
    ...
    def start_task(self, task_id: str, agent_name: str):
        ...
        self.record_metric(
            name="task_started", value=1, metric_type=MetricType.COUNTER, tags={"agent": agent_name, "task_id": task_id}
        )

        logger.debug(f"Task started: {task_id} by {agent_name}")

    def end_task(self, task_id: str, agent_name: str, success: bool, error_type: Optional[str] = None):
        """タスク終了を記録"""
        ...
        self.record_metric(
            ...
            tags={"agent": agent_name, "task_id": task_id, "success": str(success), "error_type": error_type or "none"},
        )
    ...
    def record_fix_attempt(self, fix_type: str, success: bool, duration: float, error_type: str):
        """修正試行を記録"""
        ...
        self.record_metric(
            ...
            value=duration,
            metric_type=MetricType.TIMER,
            tags={"fix_type": fix_type, "success": str(success), "error_type": error_type},
        )
    ...
    def get_all_agents_performance(self) -> Dict[str, PerformanceStats]:
        ...
        return {agent_name: self.get_agent_performance(agent_name) for agent_name in self.agent_stats.keys()}
    ...
    def generate_report(
        self, period: ReportPeriod = ReportPeriod.DAILY, start_time: Optional[datetime] = None
    ) -> Dict[str, Any]:
        """
        レポートを生成
        ...
        task_succeeded = sum(
            1 for m in period_metrics if m.name == "task_completed" and m.tags.get("success") == "True"
        )
        task_failed = task_completed - task_succeeded

        # 修正統計
        fix_attempts = sum(1 for m in period_metrics if m.name == "fix_attempt")
        fix_succeeded = sum(1 for m in period_metrics if m.name == "fix_attempt" and m.tags.get("success") == "True")

        # 平均実行時間
        durations = [m.value for m in period_metrics if m.name == "task_duration"]
    ...
    def generate_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "errors": {"distribution": self.get_error_distribution(), "top_5": dict(self.get_top_errors(5))},
            "fixes": {
                ...
                "by_type": {
                    fix_type: {
                        ...
                        "success_rate": round(
                            (stats["succeeded"] / stats["attempted"] * 100) if stats["attempted"] > 0 else 0, 2
                        ),
                    }
