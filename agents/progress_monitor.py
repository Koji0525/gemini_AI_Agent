...
class ProgressMonitor:
        ...
        try:
            ...
            try:
                ...
                batch_logs = [
                    ...
                    if str(log.get('task_id')) in batch_task_ids
                ]
    ...
    async def get_overall_summary(self) -> Dict:
        ...
        try:
            ...
            logger.info(f"✅ プロジェクト全体: {total_tasks}タスク, {overall_completion:.1f}%完了, 品質={avg_quality:.1f}")
    ...
    async def update_dashboard(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                dashboard_sheet = sheet.add_worksheet(
                    title='progress_dashboard',
                    rows=100,
                    cols=10
                )
                
                headers = [
                    'batch_id', 'goal_id', 'total_tasks', 'completed_tasks',
                    'pending_tasks', 'failed_tasks', 'completion_rate(%)',
                    'avg_quality_score', 'high_quality_count(>=8)',
                    'problem_count(<7)', 'last_updated'
                ]
