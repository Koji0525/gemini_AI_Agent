...
class DynamicTaskManager:
    ...
    def add_dynamic_task(
        self, goal_id: str, description: str, priority: str = "medium", dependencies: str = ""
    ) -> Dict[str, Any]:
        """動的にタスクを追加"""
        if not self.sheets:
        ...
        try:
            ...
            # Sheetsに追加
            self.sheets.append_rows("pm_tasks", [task_data])
    ...
    def adjust_task_priority(self, task_id: str, new_priority: str) -> Dict[str, Any]:
        """タスクの優先度を動的に調整"""
        # 実装は既存のSheets操作を使用
