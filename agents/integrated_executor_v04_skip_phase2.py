...
"""
統合実行エンジン v1.4
Phase 2（タスク分解）をスキップして、既存タスクを実行
"""
...
async def execute_goal_complete(goal_id: str, max_tasks: int = 5, skip_breakdown: bool = False):
    """ゴールを完全自動実行"""

    print(f"\n{'='*70}")
    print(f"🎯 目標 {goal_id} の完全自動実行を開始")
    if skip_breakdown:
        print("   ℹ️  Phase 2（タスク分解）をスキップ")
    print(f"{'='*70}\n")

    results = {
        "goal_id": goal_id,
        "start_time": datetime.now(),
        "phase2_success": True,  # スキップ時は成功扱い
    ...
    print(f"✅ 目標{goal_id}のpendingタスク: {task_count}件")
    ...
    print(
        f"Phase 2（タスク分解）: {'⏭️  スキップ' if skip_breakdown else '✅ 成功' if results['phase2_success'] else '❌ 失敗'}"
    )
    print(f"Phase 4-6（実行）: {'✅ 成功' if results['phase4_success'] else '❌ 失敗'}")
    print(f"完了時刻: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)

    if results["phase4_success"]:
        print("\n🎉 実行が成功しました！")
    else:
        print("\n⚠️ 実行が失敗しました")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="統合実行エンジン v1.4")
    parser.add_argument("goal_id", type=str, help="目標ID")
    parser.add_argument("--max-tasks", type=int, default=5, help="最大タスク数")
    parser.add_argument("--skip-breakdown", action="store_true", help="タスク分解をスキップ")

    args = parser.parse_args()

    await execute_goal_complete(args.goal_id, args.max_tasks, args.skip_breakdown)
