...
class WPTaxonomyAgent:
    ...
    async def list_taxonomies(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{self.wp_url}/wp-json/wp/v2/taxonomies",
                auth=self.auth,
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                standard_taxonomies = ['category', 'post_tag', 'nav_menu', 'link_category', 
                                      'post_format']
                
                custom_taxonomies = {k: v for k, v in taxonomies.items() 
                                   if k not in standard_taxonomies}
    ...
    async def verify_taxonomy(self, taxonomy: str) -> bool:
        ...
        try:
            response = requests.get(
                f"{self.wp_url}/wp-json/wp/v2/taxonomies/{taxonomy}",
                auth=self.auth,
                timeout=10
            )
