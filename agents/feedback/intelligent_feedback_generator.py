...
"""
    ...
    def _calculate_roi(self, category: str, difficulty: str, current_value: float, target_value: float) -> float:
        ...
        # カテゴリ別の重要度
        category_weight = {"reliability": 1.0, "performance": 0.8, "usability": 0.6}

        # 難易度別のコスト
        difficulty_cost = {"易": 1.0, "中": 2.0, "難": 3.0}
    ...
    def _prioritize_suggestions(self, suggestions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        sorted_suggestions = sorted(
            suggestions,
            key=lambda x: (priority_order.get(x.get("priority", "低"), 1), x.get("roi_score", 0)),
            reverse=True,
        )
