...
class LocalLLMAgent:
    ...
    def __init__(self, config: Optional[LocalLLMConfig] = None):
        ...
        self.config = config or LocalLLMConfig(
            provider=LocalLLMProvider.OLLAMA, model_name="codellama:13b-instruct", endpoint="http://localhost:11434"
        )
        ...
        if not self.is_available:
            ...
            logger.info(
                f"LocalLLMAgent initialized with {self.config.provider.value} " f"(model={self.config.model_name})"
            )
    ...
    async def generate_fix(self, error_context: Dict[str, Any], file_content: str, file_path: str) -> Dict[str, Any]:
        """
        エラーに対する修正を生成
    ...
    def _build_fix_prompt(self, error_context: Dict[str, Any], file_content: str, file_path: str) -> str:
        """修正プロンプトを構築"""
...
Provide only the JSON response, no additional text."""
    ...
    async def _query_ollama(self, prompt: str) -> str:
        ...
        payload = {
            ...
            "options": {"num_predict": self.config.max_tokens, "num_ctx": self.config.context_window},
            "stream": False,
        }
...
```"""
        ...
        try:
            response = requests.post(
                ...
                json={"model": self.config.model_name, "prompt": prompt, "stream": False},
            ...
            if response.status_code == 200:
                ...
                if json_match:
                    return {"success": True, "review": json.loads(json_match)}
...
```"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if json_match:
                    return {"success": True, "refactoring": json.loads(json_match)}

            return {"success": False, "error": "Failed to parse refactoring"}
    ...
    def pull_model(self, model_name: str) -> bool:
        ...
        try:
            ...
            response = requests.post(
                f"{self.config.endpoint}/api/pull", json={"name": model_name}, stream=True, timeout=600  # 10分
            )
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        try:
            if self.config.provider == LocalLLMProvider.OLLAMA:
                response = requests.post(
                    f"{self.config.endpoint}/api/show", json={"name": self.config.model_name}, timeout=5
                )
            ...
            return {"available": True, "provider": self.config.provider.value, "model": self.config.model_name}
...
"""
    ...
    async def test_fix():
        print("\n=== Generating Fix ===")
        result = await agent.generate_fix(error_context=error_context, file_content=file_content, file_path="app.py")
