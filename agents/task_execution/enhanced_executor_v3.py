...
class EnhancedTaskExecutorV3(EnhancedTaskExecutorV2):
    ...
    def _execute_with_details(
        self, task: Dict, details: Dict, task_types: list, task_dir: Path
    ) -> Dict:
        """詳細情報を活用した実行"""
    ...
    def _execute_testing_with_details(self, task: Dict, details: Dict, task_dir: Path) -> Dict:
        ...
        # 「既存テスト」かどうかを判定
        is_existing_test = any(
            word in details["overview"].lower()
            for word in ["既存", "existing", "すべて実行", "run all"]
        )
...
"""
        ...
        # 既存テストの場合は実行スクリプトとREADMEのみ
        summary = f'テスト実行スクリプト生成（{"既存" if is_existing_test else "新規"}{test_count}テスト、カバレッジ{coverage_target}%）'
