...
class RollbackAgent:
    ...
    def __init__(
        self, backup_dir: str = ".rollback_backup", max_snapshots: int = 100, auto_rollback_enabled: bool = True
    ):
    ...
    def create_snapshot(self, file_paths: List[str], description: str = "", tags: List[str] = None) -> str:
        """
        ファイルのスナップショットを作成
        ...
        for file_path in file_paths:
            ...
            try:
                ...
                snapshot = FileSnapshot(
                    ...
                    metadata={"file_size": len(content), "line_count": content.count("\n") + 1},
                )
    ...
    def rollback(
        self, rollback_point_id: str, reason: RollbackReason = RollbackReason.MANUAL_REQUEST, dry_run: bool = False
    ) -> RollbackResult:
        """
        指定されたロールバックポイントまで復元
    ...
    def rollback_by_time(
        self, target_time: datetime, reason: RollbackReason = RollbackReason.MANUAL_REQUEST
    ) -> RollbackResult:
        """
        ...
        # 最も近いロールバックポイントを見つける
        closest_point = min(
            self.rollback_points.values(), key=lambda p: abs((p.timestamp - target_time).total_seconds())
        )

        logger.info(
            f"Rolling back to closest point: {closest_point.id} "
            f"(target={target_time}, actual={closest_point.timestamp})"
        )

        return self.rollback(closest_point.id, reason)

    def rollback_by_commit(
        self, commit_hash: str, reason: RollbackReason = RollbackReason.MANUAL_REQUEST
    ) -> RollbackResult:
        """
        ...
        matching_points = [
            p for p in self.rollback_points.values() if p.commit_hash and p.commit_hash.startswith(commit_hash)
        ]
    ...
    def analyze_impact(self, rollback_point_id: str) -> Dict[str, Any]:
        ...
        if not result.success:
            return {"error": result.error_message, "can_rollback": False}
        ...
        for snapshot in rollback_point.snapshots:
            if not os.path.exists(snapshot.file_path):
                changes.append(
                    {"file": snapshot.file_path, "type": "missing", "description": "File will be restored from backup"}
                )
                continue
            ...
            try:
                ...
                changes.append({"file": snapshot.file_path, "type": "error", "description": str(e)})
    ...
    def should_auto_rollback(self, error_context: Dict[str, Any], test_results: Dict[str, Any]) -> bool:
        """
        自動ロールバックが必要かを判定
        ...
        # クリティカルなエラータイプ
        critical_errors = ["SyntaxError", "ImportError", "ModuleNotFoundError", "AttributeError"]
    ...
    def list_rollback_points(self, tags: List[str] = None, limit: int = 10) -> List[Dict[str, Any]]:
        """
        ロールバックポイント一覧を取得
    ...
    def _create_emergency_backup(self, file_path: str):
        """緊急バックアップを作成"""
        emergency_backup = (
            self.backup_dir / f"emergency_{Path(file_path).name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def _cleanup_old_snapshots(self):
        ...
        # 古い順にソート
        sorted_points = sorted(self.rollback_points.values(), key=lambda p: p.timestamp)
    ...
    def _save_metadata(self):
        """メタデータをファイルに保存"""
        metadata = {
            "rollback_points": {point_id: point.to_dict() for point_id, point in self.rollback_points.items()},
            "rollback_history": [result.to_dict() for result in self.rollback_history[-100:]],  # 最新100件のみ
        }
...
if __name__ == "__main__":
    ...
    # スナップショットを作成
    snapshot_id = agent.create_snapshot([test_file], description="Initial version", tags=["test", "initial"])
