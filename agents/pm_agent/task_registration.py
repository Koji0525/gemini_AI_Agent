...
class TaskRegistrationAgent:
    ...
    async def register_tasks(self, goal_id: str, tasks: List[Dict[str, Any]], detail_file_path: str = "") -> int:
        """
        タスクをpm_tasksシートに登録
        ...
        try:
            ...
            # タスクをシート形式に変換
            sheet_rows = self._convert_to_sheet_format(goal_id, tasks, next_task_id, detail_file_path)
    ...
    def _convert_to_sheet_format(
        self, goal_id: str, tasks: List[Dict[str, Any]], start_id: int, detail_file_path: str = ""
    ) -> List[List[Any]]:
        """タスクをシート形式に変換"""
        sheet_rows = []
    ...
    async def _append_to_sheet_with_range(self, rows: List[List[Any]]) -> bool:
        ...
        try:
            ...
            sheet.update(range_name=range_name, values=rows)
