...
async def show_monitoring_dashboard():
    ...
    today_tasks = [
        ...
        if task.get('created_at', '').startswith(today)
    ]
    ...
    if not today_tasks:
        ...
        for i, task in enumerate(today_tasks, 1):
            ...
            print(f"   状態: {task.get('status', 'N/A')} | 担当: {task.get('required_role', 'N/A')} | 優先度: {task.get('priority', 'N/A')}")
    ...
    recent_tasks = [
        task for task in all_tasks
        if task.get('created_at', '') >= week_ago
    ]
