...
class GoalBreakdownAgent:
    ...
    async def register_tasks_to_pm_tasks(self, goal_id: str, tasks: List[Dict[str, Any]]) -> bool:
        """タスクをpm_tasksシートに登録"""
    ...
    async def execute_goal_breakdown(self, goal_id: str = "4") -> Dict[str, Any]:
        ...
        if success:
            print("✅ 登録成功")
            return {"status": "success", "goal_id": goal_id, "tasks_generated": len(tasks)}
        else:
            print("❌ 登録失敗")
            return {"status": "error", "message": "Failed to register tasks"}
