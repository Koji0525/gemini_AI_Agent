...
class CacheManagerAgent:
    ...
    def __init__(self, cache_dir: str = ".cache", similarity_threshold: float = 0.85, max_cache_size: int = 1000):
    ...
    def cache_fix(
        self, error_context: Dict[str, Any], fix_code: str, fix_description: str, ttl_hours: int = 168
    ) -> str:
        """
        修正をキャッシュに保存
    ...
    def _find_similar_fix(self, error_context: Dict[str, Any]) -> Optional[CachedFix]:
        ...
        for cached_fix in self.fix_cache.values():
            if cached_fix.metadata.get("error_type") == error_type:
                # 類似度を計算
                similarity = self._calculate_similarity(error_message, cached_fix.metadata.get("original_message", ""))
    ...
    def get_frequent_errors(self, limit: int = 10) -> List[Tuple[str, ErrorPattern]]:
        """頻繁に発生するエラーパターンを取得"""
        sorted_patterns = sorted(self.error_patterns.items(), key=lambda x: x[1].frequency, reverse=True)

        return sorted_patterns[:limit]

    def get_best_fixes(self, limit: int = 10) -> List[CachedFix]:
        """成功率の高い修正を取得"""
        valid_fixes = [fix for fix in self.fix_cache.values() if not fix.is_expired() and fix.application_count >= 3]

        sorted_fixes = sorted(valid_fixes, key=lambda f: f.success_rate, reverse=True)
    ...
    def cleanup_expired(self) -> int:
        """有効期限切れのキャッシュを削除"""
        expired_keys = [key for key, fix in self.fix_cache.items() if fix.is_expired()]
    ...
    def cleanup_low_success_rate(self, threshold: float = 0.3) -> int:
        """成功率の低いキャッシュを削除"""
        low_success_keys = [
            key for key, fix in self.fix_cache.items() if fix.application_count >= 5 and fix.success_rate < threshold
        ]

        for key in low_success_keys:
            logger.info(f"Removing low success rate fix: {key} " f"(rate={self.fix_cache[key].success_rate:.2f})")
            del self.fix_cache[key]
    ...
    def _enforce_cache_size_limit(self):
        ...
        sorted_fixes = sorted(self.fix_cache.items(), key=lambda x: x[1].last_used)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if valid_fixes > 0:
            rates = [
                fix.success_rate
                for fix in self.fix_cache.values()
    ...
    def _save_patterns(self):
        """エラーパターンをファイルに保存"""
        try:
            patterns_data = {key: pattern.to_dict() for key, pattern in self.error_patterns.items()}
