...
class LogAnalyzerAgent:
    ...
    def __init__(self, max_log_entries: int = 10000, anomaly_threshold: float = 3.0):
        ...
        # 統計情報
        self.stats = {"total_logs": 0, "by_level": Counter(), "by_source": Counter(), "anomalies_detected": 0}
    ...
    def _initialize_error_patterns(self):
        ...
        for pattern_str, level, description in patterns:
            pattern = LogPattern(pattern=description, regex=re.compile(pattern_str), level=level)
    ...
    def _detect_anomalies(self):
        ...
        # 最近のエラーログを取得
        recent_errors = [
            entry for entry in self.log_entries if entry.timestamp >= recent_cutoff and entry.level >= LogLevel.ERROR
        ]
    ...
    def _generate_recommendations(self, error_message: str, pattern: Optional[str]) -> List[str]:
        """エラーに対する推奨事項を生成"""
        recommendations = []

        if pattern == "Connection issues":
            recommendations.extend(
                ["Check network connectivity", "Verify firewall settings", "Check if the remote service is running"]
            )
        elif pattern == "Memory issues":
            recommendations.extend(["Increase available memory", "Check for memory leaks", "Optimize resource usage"])
            ...
            recommendations.extend(["Verify file path is correct", "Check if file exists", "Check file permissions"])
        elif pattern == "Database errors":
            recommendations.extend(["Check database connection", "Verify SQL syntax", "Check database credentials"])
        else:
    ...
    def get_top_errors(self, limit: int = 10) -> List[Tuple[str, int]]:
        ...
        """
        patterns_with_freq = [
            (name, pattern.frequency) for name, pattern in self.error_patterns.items() if pattern.frequency > 0
        ]
    ...
    def generate_summary_report(self) -> Dict[str, Any]:
        ...
        # 最近の異常
        recent_anomalies = [anomaly.to_dict() for anomaly in self.detected_anomalies[-5:]]
        ...
        return {
            ...
            "top_errors": [{"pattern": name, "frequency": freq} for name, freq in top_errors],
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "avg_logs_per_source": (
                self.stats["total_logs"] / len(self.stats["by_source"]) if self.stats["by_source"] else 0
            ),
        }
