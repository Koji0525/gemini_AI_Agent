"""
プロジェクトコンテキスト分析器
目的: タスク説明から実際に必要な機能を推測し、実用ツールを生成
"""
...
class ProjectContextAnalyzer:
    ...
    def __init__(self, project_root: str = "/workspaces/gemini_AI_Agent"):
        ...
        self.context = self._analyze_project()
    ...
    def _analyze_project(self) -> Dict[str, Any]:
        ...
        # 主要ディレクトリを検出
        for dir_name in ['agents', 'tools', 'scripts']:
            dir_path = self.project_root / dir_name
            if dir_path.exists():
                context['main_modules'].append(dir_name)
        ...
        for pattern in key_patterns:
            ...
            context['key_files'].extend([str(f.relative_to(self.project_root)) for f in files[:5]])
        ...
        if tools_dir.exists():
            for file in tools_dir.glob('*.py'):
                if file.name != '__init__.py':
                    context['tools_available'].append(file.stem)
        
        return context
    
    def infer_real_requirements(self, task_description: str) -> Dict[str, Any]:
        """
        タスク説明から実際に必要な機能を推測
        
        例: "CLI基盤実装" → pm_tasks操作、タスク実行制御、ログ解析など
        """
        requirements = {
            'target_system': None,
            'required_operations': [],
        ...
        # キーワードベースの推測
        if 'cli' in task_description.lower():
            # CLIツールが必要な実際のユースケース
            requirements['target_system'] = 'task_management'
            ...
            requirements['target_system'] = 'external_integration'
            requirements['required_operations'] = [
                'Google Sheets API操作',
                'GitHub API操作',
                '認証管理'
        ...
        elif 'データ' in task_description or 'db' in task_description.lower():
            requirements['target_system'] = 'data_management'
            requirements['required_operations'] = [
                'タスクデータ管理',
                'ログデータ管理',
                'ナレッジ管理'
...
def generate_practical_cli_tool(task_id: str, description: str, context: Dict) -> Dict[str, str]:
    """
    実用的なCLIツールを生成
    
    汎用サンプルではなく、プロジェクト専用の実用ツール
    """
...
'''
    ...
    readme = f'''# 実用タスク管理CLIツール

## 🎯 これは実際に使えるツールです！

このCLIツールは、プロジェクトの実際のタスク管理を効率化します。
...
1. **手動操作の削減**: Google Sheetsを開かずにタスク管理
2. **迅速な実行**: コマンド1つでタスク実行
3. **状況把握**: 統計とログで進捗を即座に確認

## 📊 効率化の実例

**Before（手動）**:
1. Google Sheetsを開く (30秒)
2. タスクを探す (30秒)
3. run_task_with_details_v2.pyを実行 (60秒)
→ **合計120秒**
