...
class AutoCommitPushAgent:
    ...
    def _default_config(self) -> dict:
        ...
        return {
            ...
            "quality_gates": {
                ...
                "duplicate_filename_check": True,  # 新機能
            },
            "auto_fix": False,
        }
    ...
    # STEP 7: 重複ファイル名チェック（新機能）
    def step7_check_duplicate_files(self) -> bool:
        """重複ファイル名を検出"""
        self._print_step(7, "DUPLICATE FILENAME CHECK - 重複ファイル名検出")

        # プロジェクトルート以下の.pyファイルを走査
        py_files = defaultdict(list)
        excluded_dirs = self.config["excluded_dirs"]

        print(f"📂 スキャン対象: プロジェクトルート以下の.pyファイル")
        print(f"🚫 除外ディレクトリ: {', '.join(excluded_dirs)}")
        print()
        ...
        for py_file in self.project_root.rglob("*.py"):
            ...
            py_files[filename].append(py_file)

        # 重複しているファイルを検出
        duplicates = {name: paths for name, paths in py_files.items() if len(paths) > 1}
        ...
        # 重複ファイルの詳細を表示
        print(f"❌ {len(duplicates)}個の重複ファイル名が検出されました:\n")
    ...
    # STEP 9: .gitignore更新（旧STEP 8）
    def step9_update_gitignore(self) -> bool:
        """必要な除外ルールを.gitignoreに追加"""
        self._print_step(9, "UPDATE .gitignore")
    ...
    # STEP 10: README更新（旧STEP 9）
    def step10_update_readme(self) -> bool:
        """README更新（対話式）"""
        self._print_step(10, "UPDATE README")
    ...
    # STEP 11: COMMIT & PUSH（旧STEP 10）
    def step11_commit_and_push(self, message: str, push: bool = True) -> bool:
        """コミット＆プッシュ"""
        self._print_step(11, "COMMIT & PUSH")
    ...
    def run(self, commit_message: Optional[str] = None, auto_push: bool = True) -> bool:
        ...
        steps = [
            ...
            (
                ...
                self.step7_check_duplicate_files,
        ...
        # 最後にコミット＆プッシュ
        if not self.step11_commit_and_push(commit_message, auto_push):
            return False
