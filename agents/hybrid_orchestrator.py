...
class HybridFixOrchestratorAgent:
    ...
    def __init__(
        self, cache_manager, local_fix_agent, cloud_fix_agent, test_runner=None, config: Optional[Dict[str, Any]] = None
    ):
        """
        Args:
        ...
        logger.info(
            f"HybridFixOrchestratorAgent initialized " f"(strategy={self.strategy.value}, mode={self.mode.value})"
        )
    ...
    async def fix_error(self, task: BugFixTask) -> FixResult:
        ...
        logger.info(f"Starting fix for task {task.task_id} " f"(strategy={self.strategy.value})")
        ...
        try:
            ...
            if cached_fix and cached_fix.success_rate >= self.confidence_threshold:
                ...
                return FixResult(
                    ...
                    execution_time=0.0,
                    cache_hit=True,
                    modifications=[{"type": "cached", "description": cached_fix.fix_description}],
                )
        except Exception as e:
    ...
    async def _parallel_fix(self, task: BugFixTask) -> FixResult:
        ...
        # 最初に完了した結果を取得
        done, pending = await asyncio.wait([local_task, cloud_task], return_when=asyncio.FIRST_COMPLETED)
    ...
    async def _execute_local_fix(self, task: BugFixTask) -> FixResult:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self.local_fix_agent.fix_error(task.error_context, task.affected_files), timeout=self.local_timeout
            )
    ...
    async def _execute_cloud_fix(self, task: BugFixTask) -> FixResult:
        ...
        try:
            ...
            # タイムアウト付きで実行
            result = await asyncio.wait_for(
                self.cloud_fix_agent.fix_error(task.error_context, task.affected_files), timeout=self.cloud_timeout
            )
    ...
    def _update_complexity_history(self, task: BugFixTask, result: FixResult):
        ...
        # 最新10件のみ保持
        self.error_complexity_history[error_type] = self.error_complexity_history[error_type][-10:]
    ...
    async def _cache_successful_fix(self, task: BugFixTask, result: FixResult):
        ...
        try:
            if result.modifications:
                ...
                fix_code = "\n".join(mod.get("content", "") for mod in result.modifications)
...
if __name__ == "__main__":
    ...
    orchestrator = HybridFixOrchestratorAgent(
        ...
        config={"strategy": "adaptive", "mode": "hybrid", "max_retry_count": 3},
    )

    # テストタスク
    task = BugFixTask(
        task_id="test-1",
        error_context={"error_type": "ImportError", "error_message": "cannot import name 'foo'"},
