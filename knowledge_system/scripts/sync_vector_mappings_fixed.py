...
def sync_vector_mappings():
    ...
    # テーブル構造に基づいて適切なクエリを使用
    # knowledge_entries テーブルの実際のカラム名を確認してクエリを構築
    cursor.execute("PRAGMA table_info(knowledge_entries)")
    columns = [col[1] for col in cursor.fetchall()]
    print(f"🔍 knowledge_entries カラム: {columns}")

    # コンテンツカラムを特定（content または text など）
    content_column = (
        "content"
        if "content" in columns
        else "text" if "text" in columns else columns[2] if len(columns) > 2 else "content"
    )
    title_column = "title" if "title" in columns else columns[1] if len(columns) > 1 else "title"
    id_column = "id" if "id" in columns else columns[0] if len(columns) > 0 else "id"

    print(f"🔍 使用カラム: id={id_column}, title={title_column}, content={content_column}")

    # マッピングが不足しているエントリーを検出
    query = f"""
        SELECT ke.{id_column}, ke.{title_column}, ke.{content_column} 
        FROM knowledge_entries ke
        LEFT JOIN vector_mappings vm ON ke.{id_column} = vm.entry_id
    ...
    print(f"✅ ベクトルマッピング同期完了: {success_count}/{len(missing_entries)}件成功")
    check_sync_status()
