...
class BugFixTask(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"task_id": "Task-54-Fix", "target_files": ["wp_agent.py"], "priority": 8, "run_tests": True}
        }
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "modified_files": {"wp_agent.py": "def method():\n    if obj is not None:\n        ..."},
                "confidence": 0.95,
...
def create_bug_fix_task_from_exception(
    task_id: str, exception: Exception, file_path: str, line_number: int, **kwargs
) -> BugFixTask:
    """例外からバグ修正タスクを作成"""
    import traceback
    ...
    return BugFixTask(task_id=task_id, error_context=error_context, target_files=[file_path])


def create_fix_result_success(task_id: str, modified_files: List[str], generated_code: str, **kwargs) -> FixResult:
    """成功結果を作成"""
    return FixResult(
        task_id=task_id,
        success=True,
...
def create_fix_result_failure(task_id: str, error_message: str, **kwargs) -> FixResult:
    """失敗結果を作成"""
    return FixResult(task_id=task_id, success=False, error_message=error_message, **kwargs)
