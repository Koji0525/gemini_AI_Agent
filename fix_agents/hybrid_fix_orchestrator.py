...
class HybridFixOrchestrator:
    ...
    async def execute_fix_task(self, bug_fix_task: BugFixTask, strategy: Optional[FixStrategy] = None) -> FixResult:
        """
        修正タスクを実行
        ...
        try:
            ...
            logger.info(
                f"{'成功' if result.success else '失敗'} 修正タスク{'完了' if result.success else '失敗'}: {task_id} ({execution_time:.2f}秒)"
            )
    ...
    async def _execute_cloud_only(self, bug_fix_task: BugFixTask) -> FixResult:
        ...
        if self.cloud_agent is None:
            ...
            return FixResult(task_id=bug_fix_task.task_id, success=False, error_message="CloudFixAgent not available")
    ...
    def _select_best_result(
        self, local_result: Optional[FixResult], cloud_result: Optional[FixResult]
    ) -> Optional[FixResult]:
        """
        2つの修正結果から最適なものを選択
                ...
                if self.cloud_agent
                    ...
                    if self.cloud_agent
                        ...
                        if self.cloud_agent
                            ...
                            {}
                            if self.cloud_agent
                            else {} if self.cloud_agent else {} if self.cloud_agent else {} if self.cloud_agent else {}
                        )
                    )
                )
            ),
