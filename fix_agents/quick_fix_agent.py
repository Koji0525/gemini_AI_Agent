...
from data_models import BugFixTask, ErrorContextModel, FixResult, ErrorSeverity
...
class QuickFixAgent:
    ...
    def __init__(self, browser_controller, command_monitor, wp_tester=None):
        """
        初期化
    ...
    async def execute_bug_fix_task(self, bug_fix_task: BugFixTask) -> FixResult:
        ...
        try:
            ...
            if not ai_result["success"]:
                return self._create_failed_result(task_id, f"AI修正依頼失敗: {ai_result.get('error')}", start_time)

            generated_code = ai_result["generated_code"]
            logger.info(f"🤖 AI修正コード取得完了 ({len(generated_code)}文字)")

            # 3. 修正コードを検証
            validation_result = self._validate_generated_code(generated_code, bug_fix_task.error_context)

            if not validation_result["valid"]:
                return self._create_failed_result(
                    task_id, f"AI修正コード検証失敗: {validation_result['reason']}", start_time
                )
            ...
            if not apply_result["success"]:
                ...
                return self._create_failed_result(task_id, f"パッチ適用失敗: {apply_result.get('error')}", start_time)
            ...
            execution_time = (datetime.now() - start_time).total_seconds()
            return self._create_failed_result(task_id, f"実行エラー: {str(e)}", start_time)
    ...
    async def _request_ai_fix(self, fix_prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not hasattr(self.browser, "send_prompt_and_wait"):
                return {"success": False, "error": "BrowserControllerにsend_prompt_and_waitメソッドがありません"}

            success = await self.browser.send_prompt_and_wait(fix_prompt, max_wait=180)  # 3分

            if not success:
                return {"success": False, "error": "AIプロンプト送信または応答待機失敗"}
            ...
            if not response_text or len(response_text) < 50:
                return {"success": False, "error": "AI応答が空または短すぎます"}

            # コードブロックを抽出
            code_blocks = re.findall(r"```python\s*\n(.*?)```", response_text, re.DOTALL)

            if not code_blocks:
                return {"success": False, "error": "AI応答にPythonコードブロックが見つかりません"}

            # 最大のコードブロックを採用
            generated_code = max(code_blocks, key=len)

            return {"success": True, "generated_code": generated_code, "full_response": response_text}

        except Exception as e:
            logger.error(f"❌ AI修正依頼エラー: {e}")
            return {"success": False, "error": str(e)}

    def _validate_generated_code(self, generated_code: str, error_context: ErrorContextModel) -> Dict[str, Any]:
        """
        AI生成コードを検証
        ...
        if len(generated_code.strip()) < 20:
            return {"valid": False, "reason": "コードが短すぎます"}

        # 2. Pythonコードの基本構造チェック
        if not any(kw in generated_code for kw in ["def ", "class ", "import ", "from "]):
            return {"valid": False, "reason": "Pythonコードの基本構造が見つかりません"}
        ...
        if error_context.error_type == "ImportError" or error_context.error_type == "ModuleNotFoundError":
            ...
            if "import " not in generated_code and "from " not in generated_code:
                return {"valid": False, "reason": "インポートエラーの修正にインポート文がありません"}
        ...
        if open_braces != close_braces or open_parens != close_parens or open_brackets != close_brackets:
            return {"valid": False, "reason": "括弧の対応が不正です"}

        # 5. 構文チェック(簡易)
        try:
            compile(generated_code, "<string>", "exec")
        except SyntaxError as e:
            return {"valid": False, "reason": f"構文エラー: {e}"}

        return {"valid": True, "reason": "検証合格"}
    ...
    async def _apply_patch(
        self, target_file: Optional[str], generated_code: str, error_context: ErrorContextModel
    ) -> Dict[str, Any]:
        """
        生成されたコードをファイルに適用
        ...
        try:
            if not target_file:
                ...
                if error_context.error_location:
                    ...
                    return {"success": False, "error": "対象ファイルが特定できません"}

            target_path = Path(target_file)

            if not target_path.exists():
                return {"success": False, "error": f"対象ファイルが存在しません: {target_file}"}
            ...
            # Diff形式のパッチ内容を生成(簡易版)
            patch_content = self._generate_simple_diff(original_content, generated_code, target_file)

            return {"success": True, "target_file": target_file, "patch_content": patch_content}

        except Exception as e:
            logger.error(f"❌ パッチ適用エラー: {e}")
            return {"success": False, "error": str(e)}

    def _generate_simple_diff(self, original: str, modified: str, filename: str) -> str:
        """簡易Diff形式のパッチ内容を生成"""
        lines = []
    ...
    async def _run_automated_tests(self, bug_fix_task: BugFixTask) -> Dict[str, Any]:
        ...
        try:
            ...
            if not test_command:
                ...
                return {"success": True, "output": "テストコマンド未特定のためスキップ", "errors": []}

            logger.info(f"🧪 テスト実行: {test_command}")

            # コマンド実行
            result = await self.cmd_monitor.execute_command(test_command, timeout=120)
