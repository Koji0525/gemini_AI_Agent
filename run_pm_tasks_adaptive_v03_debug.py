...
async def main():
    ...
    try:
        ...
        # goal-idフィルタ
        if args.goal_id:
            print(f"DEBUG: tasks count before filter = {len(tasks)}")
            if tasks:
                print(f"DEBUG: first task keys = {list(tasks[0].keys())}")
                print(f"DEBUG: first task parent_goal_id = {tasks[0].get('parent_goal_id', 'NONE')}")
            filtered = [t for t in tasks if str(t.get("parent_goal_id", "")) == str(args.goal_id)]
            print(f"DEBUG: tasks count after filter = {len(filtered)}")
            tasks = filtered
        if args.goal_id:
