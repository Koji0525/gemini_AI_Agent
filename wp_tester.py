...
class WordPressTester:
    ...
    def _validate_php_test_code(self, code: str) -> Dict:
        ...
        # テストメソッドの存在チェック
        test_method_patterns = [r"function\s+test\w+", r"public\s+function\s+test\w+", r"public\s+function\s+test_\w+"]
        ...
        # アサーションの存在チェック
        assertion_patterns = [r"\$this->assert", r"assertEquals", r"assertTrue", r"assertFalse", r"expectException"]
    ...
    def _validate_general_test_code(self, code: str) -> Dict:
        ...
        # テスト関連キーワードのチェック
        test_keywords = ["test", "assert", "expect", "verify", "should", "check", "validate"]
