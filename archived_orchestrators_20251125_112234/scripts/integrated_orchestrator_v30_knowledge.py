...
from core_agents.human_interaction_agent_v03_github_api import \
...
# Phase 2: 自己修復システム
from agents.self_healing.retry_manager import (RetryConfig, RetryManager,
                                               RetryResult)
from agents.self_healing.utils.error_classifier import ErrorClassifier
from browser_control.browser_controller import BrowserController
from task_executor.task_coordinator_v08_self_healing import \
...
class InitializationManager:
    ...
    def __init__(self):
        self.knowledge_manager = init_knowledge_manager()
...
def init_knowledge_manager():
    """ナレッジマネージャー初期化"""
    try:
        from pathlib import Path as KPath

        import yaml

        config_path = (
            KPath(__file__).parent.parent / "knowledge_system/configuration/knowledge_config.yaml"
        )
        ...
        db_path = KPath(__file__).parent.parent / config["database"]["path"]
        index_path = KPath(__file__).parent.parent / config["vector_search"]["index_path"]

        sys.path.insert(0, str(KPath(__file__).parent.parent))
        from knowledge_system.core_agents.knowledge_manager import \
            KnowledgeManager

        return KnowledgeManager(
            str(db_path), str(index_path), config["vector_search"]["model_name"]
        )
    except Exception as e:
        print(f"ℹ️ ナレッジシステムは利用不可: {e}")
        return None
...
class IntegratedOrchestrator:
    ...
    def __init__(
        ...
        def init_wp_orchestrator():
            from agents.wordpress.specialized.wp_orchestrator import \
                WordPressOrchestrator
