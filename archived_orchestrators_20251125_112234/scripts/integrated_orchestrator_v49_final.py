"""
統合オーケストレーター v49: 最終版
PMAgentV2 + GoalEvaluatorV2 使用（TaskExecutorパターン完全適用）
"""
...
from core_agents.pm_agent_v2 import PMAgentV2
...
class IntegratedOrchestratorV49:
    """3ループ統合オーケストレーター v49（最終版）"""

    def __init__(self):
        logger.info("🚀 オーケストレーター v49 初期化（最終版）")

        self.sheets = GoogleSheetsManager()
        self.safe_sheets = SafeSheetsWrapper(self.sheets)

        # ✅ TaskExecutorパターン完全適用版を使用
        self.pm_agent = PMAgentV2(self.sheets)
        ...
        logger.info("✅ 初期化完了 - TaskExecutorパターン完全適用\n")
    ...
    async def display_status(self):
        ...
        logger.info(f"学習: {len(self.learned_patterns)}件")
        logger.info("=" * 60 + "\n")
...
async def main():
    try:
        orch = IntegratedOrchestratorV49()
