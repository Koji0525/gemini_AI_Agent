...
class PMAgent:
            ...
            if task_plan:
                ...
                if fixed_path.exists():
                    ...
                    try:
                        ...
                        logger.info(
                            f"✅ 修正済みJSONを読み込みました: タスク数={len(task_plan.get('tasks', []))}"
                        )
                        self.generated_tasks = task_plan.get("tasks", [])
                        return task_plan
                    except Exception as e:
        ...
        try:
            ...
            if json_match:
                ...
                try:
                    ...
                    logger.info(
                        f"✅ JSON解析成功（パターン1）: タスク数={len(result.get('tasks', []))}"
                    )
                    return result
                    ...
                    if not in_string:
                        if char == "{":
                            ...
                            if brace_count == 0:
                                ...
                                logger.info(
                                    f"✅ 完全なJSONオブジェクトを抽出: {len(potential_json)}文字"
                                )

                                try:
                                    result = json.loads(potential_json)
                                    logger.info(
                                        f"✅ JSON解析成功: タスク数={len(result.get('tasks', []))}"
                                    )
                                    return result
                                except json.JSONDecodeError as e:
                                    # === パート7: 解析エラーの詳細ログ ===
    ...
    def display_task_summary(self, task_plan: Dict):
        ...
        for i, task in enumerate(task_plan.get("tasks", []), 1):
            role_icon = {"design": "📐", "dev": "💻", "ui": "🎨", "review": "✅"}.get(
                task.get("required_role", "dev"), "📋"
            )
            ...
            print(
                f"   担当: {task.get('required_role', 'dev')} | 優先度: {task.get('priority', 'medium')}"
            )
...
async def main():
    ...
    parser.add_argument(
        "--goal", type=str, help="直接目標を指定する（スプレッドシートを使わない場合）"
    )
    ...
    if args.goal:
        ...
        if not goal:
            ...
            print(
                "スプレッドシートの'project_goal'シートにstatusが'active'の目標を設定してください"
            )
