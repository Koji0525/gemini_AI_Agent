...
class QualityFeedbackLoop:
    ...
    async def process_task_result(self, task, result):
        ...
        if quality_score >= self.QUALITY_THRESHOLDS['excellent']:
            ...
            return {
                'action': 'accepted',
                'message': f"✅ 高品質（{quality_score}点）- 即時採用"
            }
...
if __name__ == "__main__":
    ...
    async def test_quality_feedback():
        ...
        result2 = {
            "quality_score": 6,
            "quality_description": "コードの可読性に問題"
        }
        ...
        result3 = {
            "quality_score": 3,
            "quality_description": "機能要件を満たしていない"
        }
        
        response3 = await qfl.process_task_result(task3, result3)
        print(f"  テスト3: {response3['message']}")
        assert response3['action'] == 'retry_with_alternative', "不合格タスクが代替アプローチされていない"
