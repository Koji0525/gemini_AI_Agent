...
class ACFDevAgent:
    ...
    def is_acf_task(self, task: Dict) -> bool:
        ...
        keywords = ["acf", "advanced custom fields", "カスタムフィールド", "フィールドグループ", "field group"]
    ...
    async def process_acf_task(self, task: Dict) -> Dict:
        ...
        try:
            ...
            if not success:
                return {"success": False, "error": "開発AI: タイムアウト（ACFフィールドグループ作成）"}
...
"""
    ...
    def _save_acf_code(self, text: str, task: Dict, acf_info: Dict) -> list:
        ...
        try:
            ...
            output_files.append({"type": "ドキュメント", "path": doc_path})
            ...
            if php_code:
                ...
                output_files.append({"type": "PHPコード", "path": php_path})
            ...
            if json_data:
                ...
                output_files.append({"type": "JSON定義", "path": json_path})
            ...
            output_files.append({"type": "README", "path": readme_path})
    ...
    def _create_acf_summary(self, acf_info: Dict, output_files: list, validation_result: Dict) -> str:
        """ACFタスクのサマリーを作成"""
