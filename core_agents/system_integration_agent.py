...
class SystemIntegrationAgent:
    ...
    def __init__(
        ...
        # 統計情報
        self.stats = {"total_errors_caught": 0, "auto_fixed_errors": 0, "retry_successes": 0, "integration_failures": 0}
    ...
    def _inject_error_handlers(self):
        ...
        try:
            ...
            if hasattr(self.ma_executor, "execute_task"):
                ...
                self.ma_executor.execute_task = self._wrap_with_error_handler(original_execute, "execute_task")
                logger.info("✅ execute_taskメソッドをラップしました")

            # 他の重要なメソッドも同様にラップ
            methods_to_wrap = ["process_task", "run_single_task", "_execute_with_retry"]

            for method_name in methods_to_wrap:
                if hasattr(self.ma_executor, method_name):
                    original_method = getattr(self.ma_executor, method_name)
                    wrapped_method = self._wrap_with_error_handler(original_method, method_name)
    ...
    def _wrap_with_error_handler(self, original_method: Callable, method_name: str) -> Callable:
        """メソッドをエラーハンドラーでラップ"""
        ...
        async def wrapper(*args, **kwargs):
            ...
            while retry_count <= self.max_retry_attempts:
                try:
                    ...
                    logger.error(
                        f"❌ エラー捕捉: {method_name} " f"(試行{retry_count + 1}/{self.max_retry_attempts + 1})"
                    )
                    ...
                    if self.auto_fix_enabled and self.hybrid_system:
                        fix_result = await self._attempt_auto_fix(
                            error=e, task_id=task_id, method_name=method_name, file_path=file_path
                        )
    ...
    async def _attempt_auto_fix(
        self, error: Exception, task_id: str, method_name: str, file_path: Optional[str]
        ...
        try:
            ...
            # コンテキスト情報を収集
            context = {"method_name": method_name, "task_id": task_id, "auto_fix_attempt": True}

            # HybridFixSystemのhandle_errorを呼び出し
            fix_result = await self.hybrid_system.handle_error(
                error=error, task_id=task_id, file_path=file_path, context=context
            )
    ...
    def _wrap_task_execution_methods(self):
        ...
        try:
            ...
            agents_to_wrap = ["wp_agent", "plugin_agent", "content_writer", "acf_agent", "cpt_agent"]
    ...
    def _wrap_agent_methods(self, agent, agent_name: str):
        ...
        try:
            # 主要なメソッドを特定
            method_names = [name for name in dir(agent) if not name.startswith("_") and callable(getattr(agent, name))]

            for method_name in method_names[:5]:  # 最初の5つのみ（例）
                try:
                    original_method = getattr(agent, method_name)
                    wrapped_method = self._wrap_with_error_handler(original_method, f"{agent_name}.{method_name}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.stats["total_errors_caught"] > 0:
            auto_fix_rate = self.stats["auto_fixed_errors"] / self.stats["total_errors_caught"]

        return {**self.stats, "auto_fix_rate": auto_fix_rate, "recent_errors": self.error_history[-10:]}  # 直近10件
