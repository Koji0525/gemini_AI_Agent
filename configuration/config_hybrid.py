...
class HybridFixConfig:
    ...
    # 実行モード
    run_mode: str = field(default_factory=lambda: os.getenv("RUN_MODE", "local"))  # local, cloud, hybrid
    ...
    # ローカルAI使用フラグ
    use_local_ai: bool = field(default_factory=lambda: os.getenv("USE_LOCAL_AI", "true").lower() == "true")

    # ローカル修正タイムアウト（秒）
    local_timeout: int = field(default_factory=lambda: int(os.getenv("LOCAL_TIMEOUT", "30")))
    ...
    # クラウドモデル名
    cloud_model: str = field(default_factory=lambda: os.getenv("CLOUD_MODEL", "gpt-4o"))

    # クラウド修正タイムアウト（秒）
    cloud_timeout: int = field(default_factory=lambda: int(os.getenv("CLOUD_TIMEOUT", "120")))
    ...
    # 自動テスト有効化
    enable_auto_tests: bool = field(default_factory=lambda: os.getenv("ENABLE_AUTO_TESTS", "true").lower() == "true")

    # WordPressパス
    wp_path: str = field(default_factory=lambda: os.getenv("WP_PATH", "/var/www/html"))
    ...
    # 自動PR作成有効化
    enable_auto_pr: bool = field(default_factory=lambda: os.getenv("ENABLE_AUTO_PR", "false").lower() == "true")
    ...
    # リポジトリオーナー
    repo_owner: Optional[str] = field(default_factory=lambda: os.getenv("REPO_OWNER"))

    # リポジトリ名
    repo_name: Optional[str] = field(default_factory=lambda: os.getenv("REPO_NAME"))

    # PRの自動マージ
    github_auto_merge: bool = field(default_factory=lambda: os.getenv("GITHUB_AUTO_MERGE", "false").lower() == "true")
    ...
    # ストレージバケット名
    storage_bucket: Optional[str] = field(default_factory=lambda: os.getenv("STORAGE_BUCKET"))

    # 自動同期
    auto_sync_storage: bool = field(default_factory=lambda: os.getenv("AUTO_SYNC_STORAGE", "true").lower() == "true")
    ...
    # 最大バックアップ数
    max_backups: int = field(default_factory=lambda: int(os.getenv("MAX_BACKUPS", "10")))
    ...
    # ログファイル
    log_file: Optional[str] = field(default_factory=lambda: os.getenv("LOG_FILE", "./logs/hybrid_fix.log"))
    ...
    # 並列実行の最大数
    max_parallel_fixes: int = field(default_factory=lambda: int(os.getenv("MAX_PARALLEL_FIXES", "3")))
    ...
    def validate(self) -> bool:
        ...
        # 戦略の検証
        valid_strategies = ["LOCAL_ONLY", "CLOUD_ONLY", "LOCAL_FIRST", "CLOUD_FIRST", "PARALLEL", "ADAPTIVE"]
...
def get_config(env: str = "development") -> HybridFixConfig:
    ...
    """
    configs = {"development": DevelopmentConfig, "production": ProductionConfig, "cloud_only": CloudOnlyConfig}
